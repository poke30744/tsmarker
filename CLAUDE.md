...
- `fix_srt_gaps.py` — Post-process `.corrected.srt` to fix end-time gaps caused by whisper VAD bugs (k=0.8s merge threshold, m=0.5s extension)
...
- `speech/` — Speech-to-text (whisper.cpp via subprocess) + LLM-based marking; `whisper_stt.py` STT orchestration & SRT generation, `text_extractor.py` subtitle prep, `llm_client.py` OpenAI client, `prompt_engine.py` YAML context prompts
...
- Python: rich, scikit-learn, opencv-python, pandas, pysubs2, huggingface-hub, openai, requests, PyYAML, python-dotenv
- **whisper-cli** — must be in PATH, compiled for target CUDA architecture (see `whisper_stt.py`)
